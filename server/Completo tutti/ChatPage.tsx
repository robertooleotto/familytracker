...
import {
  ...
  PawPrint, Car, CreditCard, Phone, Cake, UtensilsCrossed, Landmark, Sparkles, GraduationCap, ShieldCheck, ChefHat, FolderOpen, HeartPulse,
} from "lucide-react";
...
const MORE_SECTIONS = [
  {
    ...
    items: [
      ...
      { id: "budget",    label: "Budget spese",  icon: Wallet,        color: "#3B82F6" },
      { id: "banking",   label: "Conto bancario",icon: Landmark,      color: "#059669" },
  ...
  {
    title: "Servizi & Ricorrenze",
    items: [
      { id: "subscriptions", label: "Abbonamenti",  icon: CreditCard,       color: "#8B5CF6" },
      { id: "contacts",      label: "Contatti casa", icon: Phone,            color: "#10B981" },
      { id: "anniversaries", label: "Compleanni",   icon: Cake,             color: "#EC4899" },
  ...
  {
    title: "Sicurezza Anziani",
    items: [
      { id: "elderly",  label: "Monitor Anziani",  icon: HeartPulse,  color: "#DC2626" },
    ],
  },
  {
    title: "Altro",
    items: [
      ...
      { id: "settings", label: "Impostazioni",  icon: Settings, color: "#6B7280" },
    ],
  },
];
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Scrollable content */}
        <div className="overflow-y-auto pb-8 px-4" style={{ maxHeight: "calc(84vh - 80px)" }}>
...
function BottomNav({
  ...
  return (
    <nav
      className="flex flex-shrink-0 border-t border-border bg-background/[0.97]"
      style={{
...
function MainApp() {
  ...
  const VALID_TABS = new Set(["briefing","map","chat","calendar","shopping","budget","tasks","deadlines","meds","geofences","settings","pets","vehicles","subscriptions","contacts","anniversaries","dinner","cucina","banking","ai","school","smartprotection","documenti","home","elderly"]);
  ...
  // Auto-navigate to banking tab when TrueLayer redirects back
  useEffect(() => {
  ...
  return (
    <AuthContext.Provider value={{ profile, token, isAuthenticated, login, logout }}>
      <div className="flex flex-col h-screen max-w-md mx-auto overflow-hidden shadow-2xl relative bg-background">
        ...
        {activeTab !== "briefing" && (
          <header
            className="flex items-center justify-between px-4 py-3 flex-shrink-0 bg-background/[0.96] border-b border-border"
