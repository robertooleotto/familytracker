...
export class DbStorage {
  ...
  async getTasksByFamily(familyId: string): Promise<(Task & { assignedProfile: Profile; createdByProfile?: Profile })[]> {
  ...
  async getVitalSigns(profileId: string, type?: string, limit = 50): Promise<any[]> {
    const conditions = [eq(vitalSigns.profileId, profileId)];
