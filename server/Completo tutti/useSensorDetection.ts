...
import {
  Heart, Phone, Pill, AlertTriangle, CheckCircle2, XCircle, Clock,
  Activity, Thermometer, Droplets, Scale, Wind, Plus, ChevronDown,
  ChevronUp, MapPin, Shield, Bell, RefreshCw, FileText, User,
} from "lucide-react";
...
function ElderlySimplifiedView() {
  ...
  const checkedIn = todayCheckin && todayCheckin.status !== "pending";

  return (
    <div className="flex-1 overflow-y-auto">
      <div className="p-4 space-y-4">
        {/* SOS Button — always visible, large */}
        <button
            ...
            <div className="flex items-center gap-3 py-2">
              ...
              <div>
                <p className="font-semibold text-green-700 dark:text-green-400">
                  {todayCheckin.status === "ok" ? "Hai segnalato: Tutto bene!" : "Hai chiesto aiuto"}
                </p>
                {todayCheckin.respondedAt && (
                  <p className="text-xs text-muted-foreground">alle {format(new Date(todayCheckin.respondedAt), "HH:mm")}</p>
                )}
              </div>
            </div>
          ) : (
            ...
            {todayMeds.map((med: any) => (
              <div key={med.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                ...
                {med.status === "taken" ? (
                  ...
                  <Button size="sm" onClick={() => medConfirmMut.mutate({ medicationId: med.medicationId, scheduledTime: med.scheduledTime, status: "taken" })}>
                    Presa!
                  </Button>
                )}
              </div>
...
function CaregiverDashboard() {
  ...
  if (isLoading) return <div className="flex-1 overflow-y-auto p-4 space-y-3">{[1,2,3].map(i => <Skeleton key={i} className="h-24 w-full rounded-2xl" />)}</div>;
      ...
      <div className="p-4 space-y-4">
        ...
        {unackAlerts && unackAlerts.length > 0 && (
          <div className="rounded-2xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 p-4 space-y-2">
            ...
            {unackAlerts.slice(0, 3).map((al: any) => (
              <div key={al.id} className="flex items-center justify-between bg-white dark:bg-red-950/20 rounded-xl px-3 py-2">
                ...
                <Button size="sm" variant="ghost" className="text-xs h-7" onClick={() => ackMut.mutate(al.id)}>Visto</Button>
              </div>
            ))}
            {unackAlerts.length > 3 && (
              <button onClick={() => setShowAlerts(true)} className="text-xs text-red-600 font-medium">Vedi tutti ({unackAlerts.length})</button>
            )}
          </div>
        )}
        ...
        {loadingDash ? (
          ...
          <>
            ...
            {dashboard.latestVitals && Object.keys(dashboard.latestVitals).length > 0 && (
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => setShowVitalForm(!showVitalForm)}>
            ...
            {/* Vital sign form */}
            {showVitalForm && <VitalSignForm profileId={activePid!} onDone={() => { setShowVitalForm(false); queryClient.invalidateQueries({ queryKey: [`/api/elderly/dashboard/${activePid}`] }); }} />}
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1">
                  {dashboard.recentCheckins.slice(0, 7).map((c: any) => (
                    <div key={c.id} className="flex items-center justify-between py-1.5 px-3 rounded-lg bg-muted/30">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => setShowEmergencyCard(!showEmergencyCard)}>
              ...
              {showEmergencyCard && <EmergencyCardForm profileId={activePid!} existing={dashboard.emergencyCard} onDone={() => { setShowEmergencyCard(false); queryClient.invalidateQueries({ queryKey: [`/api/elderly/dashboard/${activePid}`] }); }} />}
            </div>
            ...
            {dashboard.recentAlerts?.length > 0 && (
              <div className="space-y-2">
                ...
                {dashboard.recentAlerts.slice(0, 5).map((al: any) => (
                  <div key={al.id} className="flex items-start gap-2 py-2 px-3 rounded-xl border border-border bg-card">
...
function StatusCard({ label, status, detail }: { label: string; status?: "green" | "yellow" | "red"; detail: string }) {
  ...
  return (
    <div className={`rounded-xl border p-3 ${colors[s]}`}>
...
function VitalSignForm({ profileId, onDone }: { profileId: string; onDone: () => void }) {
  ...
  return (
    <div className="rounded-2xl border-2 border-primary/30 bg-card p-4 space-y-3">
      ...
      <select value={type} onChange={e => setType(e.target.value as VitalType)} className="w-full rounded-lg border border-border px-3 py-2 text-sm bg-background">
        {Object.entries(VITAL_CONFIG).map(([k, v]) => <option key={k} value={k}>{v.label} ({v.unit})</option>)}
      </select>
      <div className="flex gap-2">
        <input type="number" placeholder={cfg.hasValue2 ? "Sistolica" : cfg.label} value={value} onChange={e => setValue(e.target.value)} className="flex-1 rounded-lg border border-border px-3 py-2 text-sm bg-background" />
        {cfg.hasValue2 && <input type="number" placeholder={cfg.v2Label || "Valore 2"} value={value2} onChange={e => setValue2(e.target.value)} className="flex-1 rounded-lg border border-border px-3 py-2 text-sm bg-background" />}
      </div>
      <div className="flex gap-2">
        <Button className="flex-1" disabled={!value || mut.isPending} onClick={() => mut.mutate({ profileId, type, value, value2: value2 || undefined, unit: cfg.unit })}>
...
function EmergencyCardForm({ profileId, existing, onDone }: { profileId: string; existing?: any; onDone: () => void }) {
  ...
  const field = (label: string, key: string, placeholder?: string, type = "text") => (
    <div>
      ...
      <input type={type} value={(form as any)[key]} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))} placeholder={placeholder || label}
    ...
    <div className="rounded-2xl border-2 border-primary/30 bg-card p-4 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div>
          ...
          <select value={form.bloodType} onChange={e => setForm(f => ({ ...f, bloodType: e.target.value }))} className="w-full rounded-lg border border-border px-3 py-2 text-sm bg-background">
            <option value="">---</option>
      ...
      <div className="flex gap-2">
        <Button className="flex-1" disabled={!form.fullName || mut.isPending} onClick={() => mut.mutate({
