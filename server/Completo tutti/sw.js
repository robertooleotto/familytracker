...
const PLABEL: Record<string, string> = { tink: "Tink", saltedge: "Salt Edge", yapily: "Yapily", gocardless: "GoCardless", truelayer: "TrueLayer" };
...
export default function BankingPage() {
  ...
  useEffect(() => { const c = linked.length; if (connections !== undefined && c > prevLinkedCount.current && prevLinkedCount.current > 0) toast({ title: "Nuova banca collegata!" }); prevLinkedCount.current = c; }, [linked.length, connections]);

  // ── Mutations ────────────────────────────────────────────
  const connectMut = useMutation({
    mutationFn: (bankId: string) => apiRequest("POST", "/api/banking/unified/connect", { bankId }).then(r => r.json()),
    onSuccess: (data: any) => { queryClient.invalidateQueries({ queryKey: ["/api/banking/connections"] }); if (data?.authUrl) window.location.href = data.authUrl; },
  ...
  if (!status.configured) return (
    <div className="flex-1 overflow-y-auto p-4">
      <div className="rounded-2xl border border-border bg-muted/30 p-6 text-center space-y-4">
        <div className="w-16 h-16 rounded-2xl bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center mx-auto"><Landmark className="w-8 h-8 text-amber-600" /></div>
        <div><h3 className="font-semibold text-base mb-1">Configurazione richiesta</h3><p className="text-sm text-muted-foreground">Per collegare il conto, configura almeno un provider Open Banking.</p></div>
        <div className="text-left bg-background rounded-xl border border-border p-4 space-y-3">
          <p className="text-xs font-semibold text-muted-foreground uppercase">Opzione consigliata: Tink (gratis)</p>
          <ol className="text-sm space-y-2">
            <li className="flex gap-2"><span className="font-bold text-primary">1.</span>Registrati su <strong>console.tink.com</strong></li>
            <li className="flex gap-2"><span className="font-bold text-primary">2.</span>Crea un'app → copia <strong>Client ID</strong> e <strong>Secret</strong></li>
            <li className="flex gap-2"><span className="font-bold text-primary">3.</span>Aggiungi <code className="text-xs bg-muted px-1 rounded">TINK_CLIENT_ID</code> e <code className="text-xs bg-muted px-1 rounded">TINK_CLIENT_SECRET</code></li>
          </ol>
          <p className="text-xs text-muted-foreground">Altre opzioni: Salt Edge, Yapily, TrueLayer</p>
        </div>
        <p className="text-xs text-muted-foreground flex items-center justify-center gap-1"><Shield className="w-3 h-3" /> Sola lettura · Open Banking PSD2</p>
      </div>
    </div>
  );
        ...
        {showSearch && (
          <div className="rounded-2xl border-2 border-primary/30 bg-card p-4 space-y-3">
            ...
            <div className="max-h-80 overflow-y-auto space-y-1">
              ...
              {!searching && debouncedQ.length >= 1 && searchResults?.length === 0 && <div className="text-center py-6 text-muted-foreground"><p className="text-sm">Nessuna banca trovata per "{debouncedQ}"</p><p className="text-xs mt-1">Prova con un altro nome</p></div>}
              {!searching && searchResults && searchResults.length > 0 && <>
                ...
                {searchResults.map(bank => (
                  <button key={bank.id} onClick={() => { connectMut.mutate(bank.id); setShowSearch(false); setSearchQuery(""); }} disabled={connectMut.isPending} className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-muted/60 active:bg-muted transition-colors text-left">
                    ...
                    <div className="flex-1 min-w-0"><p className="text-sm font-medium truncate">{bank.name}</p><p className="text-xs text-muted-foreground">via {PLABEL[bank.provider] || bank.provider}</p></div>
                    <ArrowRight className="w-4 h-4 text-muted-foreground flex-shrink-0" />
                  </button>
                ))}
              </>}
              {!searching && debouncedQ.length === 0 && <div className="text-center py-6 text-muted-foreground"><Landmark className="w-8 h-8 mx-auto mb-2 opacity-30" /><p className="text-sm">Digita il nome della tua banca</p><p className="text-xs mt-1">Supportate migliaia di banche in Italia e Europa</p></div>}
            </div>
          </div>
        )}
